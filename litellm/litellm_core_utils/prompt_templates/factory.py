...
def anthropic_process_openai_file_message(
    ...
    if file_data:
        ...
        return_block_param: Union[AnthropicMessagesDocumentParam, AnthropicMessagesImageParam, AnthropicMessagesContainerUploadParam] | None = None
...
def _load_image_from_url(image_url):
    ...
    try:
        ...
        client = HTTPHandler(concurrent_limit=1)
        response = client.get(image_url, headers=IMAGE_FETCH_HEADERS)
...
class BedrockImageProcessor:
    ...
    async def get_image_details_async(image_url) -> tuple[str, str]:
        try:
            ...
            # Send a GET request to the image URL
            response = await client.get(image_url, follow_redirects=True, headers=IMAGE_FETCH_HEADERS)
            response.raise_for_status()  # Raise an exception for HTTP errors
    ...
    def get_image_details(image_url) -> tuple[str, str]:
        try:
            client = HTTPHandler(concurrent_limit=1)
            # Send a GET request to the image URL
            response = client.get(image_url, follow_redirects=True, headers=IMAGE_FETCH_HEADERS)
            response.raise_for_status()  # Raise an exception for HTTP errors
...
def _insert_assistant_continue_message(
    ...
    assistant_continue_message: Union[str, ChatCompletionAssistantMessage] | None = None,
